        ...
        def perturb_input_prehook_full(module: torch.nn.Module, inp, layer_id):
            ...
            if layer_id in self.perturbation_layers:
                ...
                if self.with_input_norm:
                    norm_vals = inp0.data[p_idices_inbatch[p_index]].view(len(p_index), -1).norm(dim=-1) ** 2  
                    p_factor_tensor = torch.from_numpy(p_factor[p_index]).float().to(inp0.device)
                    for _ in range(num_new_axises):
                      norm_vals = norm_vals.unsqueeze(-1)
                      p_factor_tensor=p_factor_tensor.unsqueeze(-1) 
                    perturb = norm_vals * self.perturbations[layer_id][p_idices[p_index]] * p_factor_tensor   
                    ...
                    for _ in range(num_new_axises):
                      p_factor_tensor=p_factor_tensor.unsqueeze(-1) 
                    perturb = self.perturbations[layer_id][p_idices[p_index]] * p_factor_tensor
