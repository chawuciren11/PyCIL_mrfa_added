...
class iCaRLmrfa(BaseLearner):
    ...
    def after_task(self):
        ...
        if self.save_task_checkpoints and self._cur_task in self.save_task_checkpoints:
          ...
          full_filename = os.path.join(  
              'checkpoints/mrfa',  
              f'{self.args["convnet_type"]}_{self.args["dataset"]}_{self.args["init_cls"]}-{self.args["increment"]}_{self.args["model_name"]}_{self._cur_task}.pkl'  
          )  
          # 确保目录存在  
          os.makedirs(os.path.dirname(full_filename), exist_ok=True)
          self.save_checkpoint(full_filename)
          #self.save_checkpoint(f'checkpoints/mrfa/{self.args["convnet_type"]}_{self.args["dataset"]}_{self.args["init_cls"]}-{self.args["increment"]}_{self.args["model_name"]}')
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        self.MRFA.register_perturb_forward_prehook(self._old_network, self.args['convnet_type'])
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (indices, inputs, targets) in enumerate(train_loader):
                ...
                if self.args.get('auto_kd', True):
                    auto_kd_factor = self._known_classes / self._total_classes
                    loss = 2 * (1 - auto_kd_factor) * loss_clf + auto_kd_factor * loss_kd
                else:
